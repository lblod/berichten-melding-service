...
export async function getAuthticationConfigurationForJob(jobUri) {
  const queryStr = `
    ${env.PREFIXES}
    SELECT DISTINCT ?authenticationConfiguration WHERE {
      ...
      ?job dgftSec:targetAuthenticationConfiguration ?authenticationConfiguration;
        a cogs:Job.
    }
  `;
  const result = await query(queryStr);
  return parseResult(result)[0].authenticationConfiguration;
}
